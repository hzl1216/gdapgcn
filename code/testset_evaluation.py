...
def get_test_priorization(dis2gene_test_true,dis2gene_test_all,feature_matrix,lp,total_top=10,batch_size=256):
    ...
    for d in dis2gene_test_all.keys():
        ...
        writer_gp.writerow([ '',d,sum(tp[total_top-1]),sum(fp[total_top-1]),sum(fn[total_top-1]),str(top_list),str(dis2gene_test_true[d]) ])
        writer_gp.writerow([ '','','','','',str([ result[i][1] for i in range(0,total_top) ]),'' ])

    #csvfile_gp.close()
    sample_length = len(tp[0])
    prec = [ [ float(tp[top][i])/(tp[top][i]+fp[top][i]) for i in range(0,sample_length) ] for top in range(0,total_top) ]
    recall = [ [ float(tp[top][i])/(tp[top][i]+fn[top][i]) for i in range(0,sample_length) ] for top in range(0,total_top) ]
    f1score = []
...
def get_test_priorization_new(dis2gene_test_true,dis2gene_test_all,feature_matrix,lp,total_top=10,batch_size=256,theta=0.8,threshold=0.8):
    '''
    :param dis2gene_test_true: the validate set
    :param dis2gene_test_all: the gene set of waiting for prediction
    :param feature_matrix: feature matrix
    ...
    :return: performances
    '''
    file_home = files_name['file_home']
    if args.cuda==True:
        lp = nn.DataParallel(lp)
        lp.cuda()
    ...
    #save the prediction result
    number = args.number
    gp_file = file_home + '/results/gene_priorization_%d.csv'%number
    df_gp = pd.DataFrame(columns=('did','tp','fp','fn','gid_predict','gid_real'))
    df_gp.to_csv(gp_file)
    csvfile_gp = open(gp_file, 'a')
    writer_gp = csv.writer(csvfile_gp)
...
def main(files_home):
    ...
    for epoch in range(args.epochs - 5, args.epochs):
        ...
        if 0:
            ...
            ap,prec,recall,f1score = get_test_priorization_new(dis2gene_test_true,dis2gene_test_all,feature_matrix,lp)
            print('Performance for number=%d epoch=%d'%(number,epoch))
            print('AP: ',ap)
            print('Prec: ',prec)
            print('Recall: ',recall)
